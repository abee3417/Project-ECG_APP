...
public class MainActivity extends AppCompatActivity {
    ...
    ArrayList<Integer> data = new ArrayList<>(512);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mHandler = new Handler(){ // 서버로 데이터 전송하기 위해 data 배열에 add
            public void handleMessage(Message msg){
                if(msg.what == MESSAGE_READ){
                    int tmpNum = 0;
                    for(int i = 0; i < 1024; i++) {
                        tmpNum = ((byte[]) msg.obj)[i]&0xff;
                        if(tmpNum != 0) {
                            data.add(tmpNum - 1);
                        }
                        else {
            ...
            while (true) {
                try {
                    ...
                    if(bytes != 0) {
                        ...
                        for(int i = bytes; i < 1024; i++) { // 버퍼 쓰레기값 초기화
                            buffer[i] = 0;
                        }
